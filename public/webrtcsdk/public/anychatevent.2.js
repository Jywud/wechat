...
function OnAnyChatRoomOnlineUser(dwUserCount, dwRoomId) {
	...
	if(userList && userList[0]){
		// if(user_open_id.length!=2)   opencamera_user_id(userList[0],1);
	}
	for(var i in userList){
		if(user_open_id.length!=2) opencamera_user_id(userList[i],1);
	}
...
function OnAnyChatUserAtRoom(dwUserId, bEnterRoom) {
	if(bEnterRoom==1){
		if(!user_open_id.length!=2)   opencamera_user_id(dwUserId,1);
